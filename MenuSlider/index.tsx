...
export const MenuSlider = () => {
  ...
  const copilotItem = itemsSlider.find(
    (item) => item.id === "co-pilot-dashboard"
  );
      ...
      <div className="flex-col sm:flex-row flex-wrap flex sm:justify-between px-6 lg:max-w-[95%] xl:max-w-[1580px] w-full mx-auto gap-6">
        <section className="flex bg-card-gray flex-1 min-w-[375px] h-[460px] rounded-2xl">
          <div className="flex flex-wrap my-12 mx-6 gap-8 w-full content-start">
              ...
              <div className="flex flex-col w-[65px]" key={flow._id}>
                <button
                  ...
                  onClick={() =>
                    redirectToApp(
                      {
                        url: `${getUrls("front")?.wf}modulo/${flow._id}?token=${
                          user?.token
                        }`,
                      },
                      toast,
                ...
                >
                  {flow.theme === "Cobrança" ? (
                    ...
                    <img src={coin} alt="coin icon" className="pl-3" />
                  ) : flow.theme === "Comercial" ? (
                    <img src={cart} alt="cart icon" className="pl-3" />
                  ) : (
                    <></>
                  )}
                </button>
            ...
            {user &&
            user.current_client &&
            user.current_client === "c8682884-0928-4664-a609-7c9a984c71c1"
             ? (
            ...
            {user &&
            user.current_client &&
            Object.keys(clientsWithAccessToCAP).includes(user.current_client) &&
            portalItem ? (
              <div className="flex flex-col w-[65px]">
                <button
                  className="bg-primary-500 w-[65px] h-[65px] shrink-0 rounded-lg mr-1 hover:bg-primary-600 hover:scale-105"
                  ...
                  onClick={() =>
                    redirectToApp(
                      { url: portalItem.url, disabled: portalItem.disabled },
                      toast,
                      navigate
                    )
                  }
              ...
              "
              >
                Você não possui nenhum aplicativo criado
              </p>
            ) : (
              <></>
            )}
            ...
            {[
              {
                redirect: {
                  ...
                  disabled: !(
                    ...
                    user.permitions_slug.includes(PossiblePermissions.ISAC)
                  ),
                },
                icon: <FlowIcon w={34} h={34} />,
                name: "ISAC",
              },
              {
                redirect: {
              ...
              <div className="flex flex-col w-[65px]" key={item.name}>
                <button
                  ...
                  onClick={() => {
                    if (item.redirect)
                      redirectToApp(item.redirect, toast, navigate);
          ...
          <div className="flex flex-wrap my-12 mx-6 gap-8 w-full content-start">
            {[
              ...
              <div className="flex flex-col w-[65px]" key={item.name}>
                ...
                <span className="text-primary-200 text-xs text-center truncate hover:whitespace-normal mt-3">
                  {item.name}
                </span>
              </div>
            ))}
